...
def _pandoc_impl(ctx):
    ...
    cli_args = [ctx.expand_location(a, ctx.attr.data) for a in ctx.attr.options]
    ...
    data_deps = depset(transitive = [d.files for d in ctx.attr.data])
    ctx.actions.run(
        mnemonic = "Pandoc",
        ...
        inputs = depset(
            ...
            transitive = [toolchain.pandoc.files, data_deps],
        ),
